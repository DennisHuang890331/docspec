...
5. **Descend, crystallizing section-by-section** — set each section's `brief` (audience / depth / breadth / forbidden), writing ONLY the field that differs from the ancestor chain; always fill `forbidden`. Write each section's one-line `concept` as its ROLE in the argument (each conclusion the next section's premise), then split MECE or call it a leaf. Note when content should be a table, list, or **diagram** (a drawio image authored by a delegated `dspx-diagram` subagent — never TikZ/mermaid). **Every normative decision needs the human's INDIVIDUAL confirmation** (three-role doctrine: the human is the only source of decisions — you draft, the human rules; batch nodding is not confirmation): present each decision, wait for the ruling, only then `docspec put <section> decisions --change <id>`, and at that moment record the human's ruling (gist or verbatim) into the change's `notes.md`. A value another section/document will obey or restate is a NORMATIVE VALUE — raise it as a decision (never leave it as a bare number in prose/material). `docspec status` shows a section as developing until its required fields are complete.
...
- Don't auto-crystallize — and NEVER put a normative decision without the human's individual confirmation behind it; record the ruling into notes.md as it happens.
