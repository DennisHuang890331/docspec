...
def _retired_sections(layout) -> list[dict]:
...
def _retired_decisions(layout) -> list[dict]:
    ...
    for leaf in leaves:
        for e in leaf.history:
            if not e.get("id") or e.get("kind") == "section":   # section 退場走 _retired_sections
                continue
            out.append({
...
def _run_retired(layout, prefix: str | None, as_json: bool) -> int:
    """`docspec status --retired [<prefix>]`：列出/查詢已退場的章節與決策（掃封存區＋活節 history）。"""
    sections = _retired_sections(layout)
    decisions = _retired_decisions(layout)
    if prefix:
        p = prefix.strip("/")
