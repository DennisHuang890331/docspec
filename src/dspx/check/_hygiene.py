...
兩類都是「疑似」狀態（可能正在施工），WARN 提示人裁決、不當閘門：
  ...
  - 死資料夾：非 `_` 前綴、但不含 article.yaml（dossier 案卷）/concept.yaml/group.yaml 且無有效後代——
...
_ALIVE_MARKERS = ("concept.yaml", "group.yaml", "article.yaml")   # article.yaml＝dossier 案卷
...
def _scan_hygiene(layout) -> list[str]:
    ...
    for p in dead:
        ...
        rel = p.relative_to(corpus)
        warnings.append(
            f"corpus/{rel.as_posix()}: dead folder — no article.yaml/concept.yaml/group.yaml "
