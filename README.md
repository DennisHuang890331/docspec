...
**圖表 = drawio 圖片**：圖在 `draft` 時由委派的 subagent（載入 `dspx-diagram` skill）用 draw.io 畫成 `.drawio`＋渲成 SVG，嵌進交付物（兩條軌都吃同一張圖）。draw.io 是選用資產，需要時跑 `docspec setup --with-drawio` 裝受控可攜版。
